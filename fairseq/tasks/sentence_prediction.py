...
class SentencePredictionTask(FairseqTask):
    ...
    def __init__(self, args, data_dictionary, label_dictionary):
        ...
        self.domains = ['WB', 'news', 'reviews', 'bio', 'cs']
        self.args = args
        #if args.use_domain_prompt:
        # print("use domain prompt")
        for domain in self.domains:
            self.domain_dict_idx[domain] = self.dictionary.add_symbol('<'+domain+'>')
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        if self.args.init_token is not None:
            print(self.domain_dict_idx.get('WB', None))
            input0 = PrependTokenDataset(input0, self.args.init_token, self.domain_dict_idx.get('WB', None))
        ...
        if input1 is None:
            ...
            if self.args.separator_token is not None:
                ...
                domain_token = self.domain_dict_idx.get('cs', None) if self.args.data.find("MNLI") >= 0 else None
                print(domain_token)
                '''
