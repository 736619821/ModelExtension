...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False):
        ...
        if self.args.expand_hidden:
            for h in range(0, 384):
                setattr(self, "self_attn_layer_normal_" + str(384 + h) , LayerNorm(384 + h, export=export))
                setattr(self, "self_attn_layer_normal_" + str(384 - h), LayerNorm(384 - h, export=export))
    ...
    def maybe_layer_norm(self, layer_norm, x, before=False, after=False):
        assert before ^ after
        if after ^ self.normalize_before:
            if self.args.expand_hidden and self.step < 384:
                #print("deal with self_attn_layer_normal_", self.step)
                y = x.split([384 + self.step, 384-self.step], dim=2)
                fisrt_param = getattr(self, "self_attn_layer_normal_" + str(384 + self.step))(y[0])
                exp_param = getattr(self, "self_attn_layer_normal_" + str(384 - self.step))(y[1])    
                tmp = []
                tmp.append(fisrt_param)
                tmp.append(exp_param)
                new_param = torch.cat(tmp, dim=2, out=None)
                #print(new_param)
