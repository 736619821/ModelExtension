...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        print("TransformerDecoder: ",  embed_tokens, no_encoder_attn )
        super().__init__(dictionary)
        self.register_buffer('version', torch.Tensor([3]))
        self.args = args
        self.step = 0
    ...
    def extract_features(
        ...
        **unused,
        ...
        if self.args.expand_layer:
            # decoder layers with redul
            max_update_step = self.args.max_update * 16
            if self.step < max_update_step:
                layer_ratio = 1.0 - float(self.step) / max_update_step
            else:
                layer_ratio = 0
            ...
            for idx, layer in enumerate(self.layers):
                ...
                layer_num = len(inner_states)
                if layer_num != 1 and layer_num % 2 == 1:
                    x = layer_ratio * inner_states[layer_num-2] + (1 - layer_ratio) * inner_states[layer_num-1]
                x, layer_attn = layer(
                    x,
                    encoder_state,
        ...
        if self.layer_norm:
            if self.args.expand_hidden and self.trainstep < 384:
                #print("deal with layer_norm", self.step)
                y = x.split([384 + self.trainstep, 384-self.trainstep], dim=2)
                fisrt_param = getattr(self, "layer_norm_add_" + str(384 + self.trainstep))(y[0])
                exp_param = getattr(self, "layer_norm_add_" + str(384 - self.trainstep))(y[1])    
                tmp = []
                tmp.append(fisrt_param)
                tmp.append(exp_param)
                x = torch.cat(tmp, dim=2, out=None)
            else:    
              x = self.layer_norm(x)
    ...
    def output_layer(self, features, **kwargs):
        ...
        if self.adaptive_softmax is None:
            ...
            if self.share_input_output_embed:
                return F.linear(features, self.embed_tokens.weight[:self.args.vocab_size, ])
            else:
